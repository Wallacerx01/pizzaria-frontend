...
export function OrderModal({ onClose, orderId, token }: OrderModalProps) {
  ...
  return (
    <Dialog open={orderId !== null} onOpenChange={() => onClose()}>
      <DialogContent className="p-6 bg-app-card text-white max-w-2xl rounded-md border border-app">
        <DialogHeader>
        ...
        <DialogFooter className="flex flex-row gap-3 sm:gap-3">
          <Button
            variant="outline"
            className="flex-1 border-app-border hover:bg-transparent bg-transparent text-white hover:text-white rounded-md"
          >
            Cancelar pedido
          </Button>
          <Button
            className="flex-1 bg-brand-primary hover:bg-brand-primary/90 text-white font-semibold rounded-md"
            disabled={loading}
